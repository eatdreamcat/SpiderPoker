...
export default class BubbleMove extends cc.Component {
  ...
  get isShooterState() {
    return (
      this.node.getParent().name == "Shooter" &&
  ...
  checkCollisionBubble() {
    ...
    for (let index of bubbleIndexes) {
      ...
      if (
        CMath.Distance(bubblePos, bubble.node.position) <=
        2 * this.radis * 0.8
      ) {
        /**
         * 计算最近的落点位置
        ...
        if (this.bubble.Type == SpecialType.Magic) {
          ...
          bubble.Type == SpecialType.Magic &&
          bubble.node.active &&
          this.bubble.Type != SpecialType.Horce
        ) {
  ...
  scaleBubble(x: number, y: number) {
    ...
    this.node.runAction(
      cc.sequence(cc.scaleTo(0.1, x, y), cc.scaleTo(0.1, 1, 1))
    );
  }
    ...
    if (this.isShooterState || this.isDrop) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (this.isDrop && pos.y <= DropBorder) {
          ...
          let score = this.bubble.DoubleScore
            ? BubbleDropScore * 2
            : BubbleDropScore;
          Game.addScore(
            this.bubble.Color,
