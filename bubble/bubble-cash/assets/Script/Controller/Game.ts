...
class GameCtrl {
  ...
  public start() {
    ...
    for (let i = 0; i < 10; i++) {
      this.season =
        SeasonPool[Math.floor(CMath.getRandom(0, SeasonPool.length))];
    }
  }
  ...
  public updateCollisionIndexes() {
    ...
    for (
      let i = this.startIndex - MatrixSize;
      i < this.startIndex - MatrixSize + UseSize;
      i++
    ) {}
  ...
  checkOutOfMove() {
    ...
    if (lastBubble) {
      let pos = CMath.ConvertToNodeSpaceAR(
        lastBubble.node,
        lastBubble.node.getParent().getParent()
      );
  ...
  public checkClear(
    ...
    if (
      this.clearIndex.length < ClearCountLimit &&
      checkBubble.Type != SpecialType.Horce
    ) {
      this.clearIndex.length = 0;
      this.streak = 0;
      return;
