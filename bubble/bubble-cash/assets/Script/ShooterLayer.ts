...
export default class ShooterLayer extends cc.Component {
  ...
  updatePoint(k: number, direction: number) {
    ...
    if (k === null) {
      for (let i = 0; i < childrenCount; i++) {
        ...
        child.getComponent(
          cc.Sprite
        ).spriteFrame = this.PointAtlas.getSpriteFrame(
          PointColor[this.Shooter.children[0].getComponent(Bubble).Color]
        );
      }
    } else {
      let dgree = Math.atan(k);
  ...
  onTouchStart(e: cc.Event.EventTouch) {
    ...
    if (this.curTouchID != null) return;
    this.curTouchID = e.getID();

    this.updateStarPosition(e.getLocation());
    this.ShooterStar.active = true;
    this.Point.active = true;
    this.ShooterStar.getComponent(
      cc.Sprite
    ).spriteFrame = this.PointAtlas.getSpriteFrame(
      SignColor[this.Shooter.children[0].getComponent(Bubble).Color]
    );
  }
